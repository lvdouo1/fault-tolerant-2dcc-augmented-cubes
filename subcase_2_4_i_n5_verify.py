...
#!/usr/bin/env python3
"""
Verification code for Theorem 4.1, Subcase 2.4(i), n = 5
...
For every 6-edge fault set F_R in AQ_4 such that delta(AQ_4 - F_R) >= 2,
the graph AQ_4 - F_R contains a 15-cycle.

This is exactly the only place where the proof for n=5 needs a computer
in Subcase 2.4(i): once such a 15-cycle A exists in R-F_R, the rest of the
subcase follows from the already-cited Proposition 2.8 on the L-side.
...
Method:
  ...
  3. Given a candidate F_R, test whether AQ_4 - F_R still has a 15-cycle.
     If it does, add a blocking constraint (and all its automorphic images).
...
On a normal laptop this should finish quickly.
"""
...
def find_hamiltonian_cycle_excluding_vertex(
    ...
    # DP[mask][u] = predecessor of u on a path that starts at 'start',
    # ends at u, and visits exactly the vertices encoded by 'mask'
    # among 'others'; the endpoint u itself is included in mask.
...
def solve_counterexample_milp(
    ...
    Solve the MILP that looks for a counterexample F_R:
      - exactly 6 faulty edges,
      - at most 5 faulty incident edges at every vertex
        (equivalently internal degree at least 2),
...
def verify_subcase_2_4_i_n5(verbose: bool = True) -> bool:
    ...
    while True:
        ...
        if res.status != 0:
            if verbose:
                print(f"[OK] No counterexample exists.")
...
if __name__ == "__main__":
    ok = verify_subcase_2_4_i_n5(verbose=True)
    if ok:
        print(
            "\nConclusion: for every 6-edge fault set F_R in AQ_4 with "
            "delta(AQ_4 - F_R) >= 2, the graph AQ_4 - F_R contains a 15-cycle."
        )
        print(
            "Hence the n=5 part of Subcase 2.4(i) is verified."
        )
    else:
        raise SystemExit(1)

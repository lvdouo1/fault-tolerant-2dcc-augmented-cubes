#!/usr/bin/env python3
"""
Computer-assisted verification for Theorem 4.1, Case 2.3.1, n = 5.

Statement verified
------------------
For every set F_R of six faulty edges in AQ_4 such that
...
The master MILP selects six faulty edges, enforces minimum remaining
...
def verify_case_2_3_1_n5(verbose: bool = True) -> bool:
...
if __name__ == "__main__":
    verified = verify_case_2_3_1_n5(verbose=True)
    if not verified:
        raise SystemExit(1)
    print(
        "\nConclusion: for every six-edge fault set F_R in AQ_4 with "
        "delta(AQ_4 - F_R) >= 2, AQ_4 - F_R contains a 15-cycle."
    )
    print("Hence the n=5 part of Case 2.3.1 is verified.")
